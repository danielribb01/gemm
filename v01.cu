...
void neoGemm(torch::Tensor A, torch::Tensor B, torch::Tensor C) {
    ...
    // Corrigir tipos de ponteiros para bf16*
    __nv_bfloat16* data_ptr_A = reinterpret_cast<__nv_bfloat16*>(A.data_ptr<at::BFloat16>());
    __nv_bfloat16* data_ptr_B = reinterpret_cast<__nv_bfloat16*>(B.data_ptr<at::BFloat16>());
    __nv_bfloat16* data_ptr_C = reinterpret_cast<__nv_bfloat16*>(C.data_ptr<at::BFloat16>());
    int prev_m, prev_n, prev_k;
    if (!d_tma_map_A) {
        // Passar ponteiros corretos ao invés de tensors
        d_tma_map_A = allocate_and_create_tensor_map<BM, BK>(data_ptr_A, M / BM, K / BK);
        d_tma_map_B = allocate_and_create_tensor_map<BN, BK>(data_ptr_B, N / BN, K / BK);
        prev_m = M;
        prev_n = N;
        prev_k = K;
    }
    ...
    gemm<
        ...
        <<<(M/BM) * (N/BN), NUM_THREADS>>>(M, N, K, data_ptr_C, d_tma_map_A, d_tma_map_B);
}
